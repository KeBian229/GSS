...
from net.models import LeNet_5 as LeNet, StructureTensors
import util
                ...
                if args.reg==3:
                    ...
                    for name, param in model.named_parameters():
                        if param.requires_grad and 'weight' in name:
                            ...
                            if len(weights_in) == 2:
                                input_neurons = torch.sum(torch.abs(weights_in)**2, dim=1)
                            elif len(weights_in) == 4:
                                input_neurons = torch.sum(torch.abs(weights_in)**2, dim=1)
                            
                                + torch.sum(torch.abs(weights_out)**2, dim=0)
                            a = weights_group**0.25 / torch.sqrt(torch.sum(torch.abs(weights_group**0.5)))
                    ...
                    for name, param in model.named_parameters():
                        if param.requires_grad and 'weight' in name and torch.sum(torch.abs(param))>0 and len(param)>=2:
                            ...
                            if len(weights_in.size()) == 2:
                                ...
                                output_neurons = torch.sum(torch.abs(weights_out)**2, dim=(0,2,3))
                            if len(output_neurons) == len(input_neurons):
            ...
            if args.reg==4:
                ...
                for name, param in model.named_parameters():
                    if param.requires_grad and 'weight' in name:
                        ...
                        weights_out = param  
                        layer_name = name.split('.')[0]
                        if len(weights_in.size()) == 4:
                            weights_group = torch.sum(torch.abs(weights_in)**2, dim=(1,2,3))
                        elif len(weights_in.size()) == 2:
                            weights_group = torch.sum(torch.abs(weights_in)**2, dim=1)
                        if len(weights_out.size()) == 4:
                            weights_group += torch.sum(torch.abs(weights_out)**2, dim=(0,2,3))
                        elif len(weights_out.size()) == 2:
                            weights_group += torch.sum(torch.abs(weights_out)**2, dim=1)
                        
                        a = weights_group / torch.sqrt(torch.sum(torch.abs(weights_group)))
                        layer_tensors.layer_tensors[layer_name] = a
