...
from net.models import LeNet, ElementTensors
import util
...
def train(epochs, decay=0, threshold=0.0):
    ...
    if args.reg == 6:
        layer_tensors = ElementTensors(model)
                    ...
                    for name, param in model.named_parameters():
                        if param.requires_grad and 'weight' in name:
                            ...
                            if weights_in.size(0) == weights_out.size(1):
                                ...
                                a = layer_tensors.layer_tensors[layer_name]
                                b = torch.where(a != 0, weights_group / a , torch.zeros_like(a))
                                reg += torch.sum(torch.abs(b)**p)
                                
                            # else:
                            #     a = layer_tensors.layer_tensors[layer_name]
                                
                            #     b = torch.where(a != 0, weights_out / a , torch.zeros_like(a)) 
            ...
            if args.reg == 6:
                ...
                for name, param in model.named_parameters():
                    if param.requires_grad and 'weight' in name:
                        ...
                        constrain_in = torch.abs(weights_in)**(p/(p+q))/(torch.sum(torch.abs(weights_in)**(p*q/(p+q)))**(1/q))
                        constrain_out = torch.abs(weights_out)**(p/(p+q))/(torch.sum(torch.abs(weights_out)**(p*q/(p+q)))**(1/q))
                        if weights_in.size(0) == weights_out.size(1):
                            a = torch.cat((constrain_in, constrain_out.t()), dim=1)
                            layer_tensors.layer_tensors[layer_name] = a
                            
                        # else:
                        #     layer_tensors.layer_tensors[layer_name] = constrain_out
                        #     layer_tensors.layer_tensors[layer_name + '2'] = constrain_in
                        #     c = torch.sum(abs(constrain_out)**q)
...
if __name__ == '__main__':
    ...
    accuracy = test()
    if args.reg == 6:
        torch.save(model.state_dict(), 'saves/elt_'+str(args.decay)+'_'+str(args.reg)+'_'+str(args.p)+'_'+str(args.q)+'.pth')
    else:
    ...
    print("total_flops: ", util.cal_model_flops(model,input))
